create or replace function upd_sal_bonus_project_sys(p_sal_date in date, project_name in varchar2) return varchar2 is
...
begin
  ...
  LOOP
    FETCH C_emps INTO p_emp_id;
          ...
          new_sal_val:= p_base_salary_to_compute+sal_bonus;
          ...
          where emp_id = p_emp_id and EXTRACT(MONTH FROM sal_date)|| '-' || EXTRACT(YEAR FROM sal_date) = 
                 EXTRACT(MONTH FROM p_sal_date) || '-' || EXTRACT(YEAR FROM p_sal_date);
          commit;
  END LOOP;
  CLOSE C_emps; 
  return(FunctionResult);
end upd_sal_bonus_project_sys;
/
